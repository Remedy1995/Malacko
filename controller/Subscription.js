...
const UserSubscriptionInfo = async (req, res) => {
  ...
  try {
    const userSubInfo = await Subscription.findOne({user : ObjectId(userId)}).populate({
      path : 'user',
      select : '-password -createdAt -_id -__v'
    }).select('-__v').exec();
  
    console.log('my user subscriber info', userSubInfo)
    
    // const concatObject = [...searchUser, ...orderInformation]
    //console.log('ordered informamtion', concatObject)
    var newSuccess = new Response(200, userSubInfo, 'Success');
...
module.exports = { LoginUser, createNewUser, createNewCustomer, UserSubscriptionInfo };